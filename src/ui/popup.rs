...
pub fn draw(frame: &mut Frame, screen: Rect, input_area: Rect, app: &App, p: &Popup) {
    ...
    let y_bottom = input_area.y.saturating_sub(slide);
    let y = y_bottom.saturating_sub(h).max(screen.y);
    let x = screen.x + 1;
    let rect = Rect {
        x,
        y,
        width: w.min(screen.width.saturating_sub(x - screen.x + 1)),
    ...
    for (i, cmd) in matches.iter().take(rows as usize).enumerate() {
        ...
        let name_color = if selected { t.accent } else { t.text };
        let mut spans = vec![
            Span::styled(
                if selected { "› " } else { "  " }.to_string(),
                Style::default().fg(t.fade(t.accent, t_in)),
            ),
            Span::styled(
                label,
                Style::default()
        ...
        let line = if selected {
            ...
            Line::from(spans).style(Style::default().bg(t.fade(t.accent, 0.10 * t_in)))
        } else {
            line
        };
        lines.push(line);
